  ...
  return (
    <motion.header style={{ background, boxShadow: shadow }} className="fixed left-1/2 top-4 z-50 w-[min(1120px,calc(100%-24px))] -translate-x-1/2 rounded-[1.8rem] border border-white/70 px-4 py-3 backdrop-blur-xl lg:rounded-full">
      <nav className="flex items-center justify-between gap-4" aria-label="Main navigation">
        <a href="#home" onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-full text-ink focus-visible:outline focus-visible:outline-3 focus-visible:outline-coral">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {navItems.map(([label, href, Icon]) => (
            <a key={label} href={href} className={`group flex items-center gap-1 rounded-full px-3 py-2 text-sm font-bold transition focus-visible:outline focus-visible:outline-3 focus-visible:outline-sky ${active === href ? "bg-sun text-ink" : "text-ink/80 hover:bg-white hover:text-ink"}`}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mobile-menu lg:hidden">
            {navItems.map(([label, href, Icon]) => (
              <a key={label} href={href} onClick={() => setOpen(false)} className={active === href ? "active" : ""}>
